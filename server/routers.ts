...
import { getGoogleCredentialsByUserId, upsertGoogleCredentials, getReviewsByUserId, getResponsesByUserId, getTrustedshopCredentialsByUserId, upsertTrustedshopCredentials, getTrustedshopResponsesByUserId } from "./db";
...
export const appRouter = router({
  ...
  googleCredentials: router({
    ...
    save: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        // Validate inputs
        if (!input.clientId?.trim() || !input.clientSecret?.trim() || !input.refreshToken?.trim() || !input.businessProfileId?.trim()) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "All fields are required" });
        }
  ...
  // Google Reviews processing (admin-only)
    ...
    setupCron: adminProcedure.mutation(async ({ ctx }) => {
      try {
        ...
        const job = await createHeartbeatJob(
          {
            name: "google-reviews-processor",
            description: "Process Google reviews every 10 minutes",
            path: "/api/scheduled/process-reviews",
            cron: "0 */10 * * * *",
          },
          ""
        );
  ...
  // TrustedShop Reviews agent routers (admin-only)
  trustedshop: router({
